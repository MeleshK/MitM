...
def checkRequestHeaders(flow, headers, results):
	if 'User-Agent' in headers.keys():
		if headers['User-Agent'][:22] == 'AndroidDownloadManager':
			...
			if ('referer' in headers.keys() and
							headers['referer'].find('android-app://com.google.android.googlequicksearchbox') == 0):
...
def checkGetURL(flow, results):
	...
	# Google Ping
		...
		type = 'System Info: Model'
		info = AppDefault.findFormEntry(flow.requestContent, 'brand') \
			+ ' ' + AppDefault.findFormEntry(flow.requestContent, 'model')
		results.append(Result.Result(flow, type, info))
...
def checkPostURL(flow, results):
	...
	# Android Check-in
	elif (flow.url == 'https://android.googleapis.com/checkin' or
		  			flow.url == 'https://android.clients.google.com/checkin'):
